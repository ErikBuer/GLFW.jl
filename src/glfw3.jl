...
struct GLFWError <: Exception
    ...
    GLFWError(code::Integer, desc) = new(try
            ErrorCode(code)
        catch
            code
        end, desc)
...
function Init_WithPlatform(; platform::Platform=@static Sys.islinux() ? PLATFORM_X11 : ANY_PLATFORM)
    require_main_thread()
...
# Error handling - managed in callback.jl
...
# Monitor callback - static compilation stub
function SetMonitorCallback(callback)
    require_main_thread()
    # Stub - no dynamic monitoring in static mode
    return nothing
end
SetMonitorCallback(::Nothing) = nothing
...
function CreateWindow(width::Integer, height::Integer, title::AbstractString, monitor::Monitor=Monitor(C_NULL), share::Window=Window(C_NULL))
    ...
    # Static compilation - no dynamic callback management
    window
end
...
function DestroyWindow(window::Window)
    ...
    # Static compilation - no callback cleanup needed
end
...
# Window event callbacks - static compilation stubs
SetWindowPosCallback(window::Window, callback) = nothing
SetWindowSizeCallback(window::Window, callback) = nothing
SetWindowCloseCallback(window::Window, callback) = nothing
SetWindowRefreshCallback(window::Window, callback) = nothing
SetWindowFocusCallback(window::Window, callback) = nothing
SetWindowIconifyCallback(window::Window, callback) = nothing
SetFramebufferSizeCallback(window::Window, callback) = nothing
SetWindowContentScaleCallback(window::Window, callback) = nothing
SetWindowMaximizeCallback(window::Window, callback) = nothing

# Set all callback functions to nothing (stubs)
SetWindowPosCallback(window::Window, ::Nothing) = nothing
SetWindowSizeCallback(window::Window, ::Nothing) = nothing
SetWindowCloseCallback(window::Window, ::Nothing) = nothing
SetWindowRefreshCallback(window::Window, ::Nothing) = nothing
SetWindowFocusCallback(window::Window, ::Nothing) = nothing
SetWindowIconifyCallback(window::Window, ::Nothing) = nothing
SetFramebufferSizeCallback(window::Window, ::Nothing) = nothing
SetWindowContentScaleCallback(window::Window, ::Nothing) = nothing
SetWindowMaximizeCallback(window::Window, ::Nothing) = nothing
...
# Joystick callback - static compilation stub
function SetJoystickCallback(callback)
    require_main_thread()
    # Stub - no dynamic joystick monitoring in static mode
    return nothing
end
SetJoystickCallback(::Nothing) = nothing
...
function standard_window_hints()
    [
        (SAMPLES, 0),
        (DEPTH_BITS, 0), (ALPHA_BITS, 8),
        (RED_BITS, 8),
        (GREEN_BITS, 8),
        (BLUE_BITS, 8), (STENCIL_BITS, 0),
        (AUX_BUFFERS, 0)
    ]
end
